...
const Mypage = () => {
  ...
  const onChangeEditor = useCallback((id) => {
  ...
  const onChangeSeeBefore = useCallback((id) => {
  ...
  const renderStatus = useCallback((value) => {
    if (value === Status.ACTIVE) return <p className="formatnotColor complete">{Languages.text.complete}</p>
    else if (value === Status.INACTIVE) return <><p className="formatnotColor free">{Languages.text.free}</p><p className="autodelete">{Languages.text.autoDelete}</p></>
